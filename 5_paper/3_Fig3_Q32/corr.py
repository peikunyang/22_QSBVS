...
def get_corr_files(root, map_name, pdbid, suffix):
    folder = root / map_name / f"{pdbid}_{suffix}"

    if not folder.is_dir():
        return []

    return [p for p in sorted(folder.iterdir(), key=natural_key) if p.is_file()]


def find_rank_file(root, map_name, pdbid):
    folder = root / map_name / f"{pdbid}_0"

    candidates = [
        folder / f"{pdbid}_0_{pdbid}",
        folder / f"{pdbid}_0_{pdbid}.ene",
    ]
    ...
    matches = sorted(folder.glob(f"{pdbid}_0_{pdbid}*"), key=natural_key)
...
def get_fixed_reference_file(root, map_name, target_file):
    relative_path = target_file.relative_to(root / map_name)
    ref_file = root / "map_0" / relative_path
    ...
    with open(output_3, "w", encoding="utf-8") as detail_f:
        ...
        for map_idx, map_name in enumerate(map_names, start=1):
            ...
            for pdb_idx, pdbid in enumerate(PDBIDS, start=1):
                ...
                original_rank_file = find_rank_file(root, map_name, pdbid)
                fixed_rank_file = find_rank_file(root, map_name, pdbid)
                ...
                if original_rank_file is not None:
                    original_rank_rows = read_rows(original_rank_file)
                    original_ranks = []
                    original_totals = []

                    for sort_col in SORT_COLS:
                        rank, total = rank_target_and_total(original_rank_rows, pdbid, sort_col)
                        original_ranks.append(rank)
                        original_totals.append(total)

                    del original_rank_rows

                if fixed_rank_file is not None:
                    fixed_rank_rows = read_rows(fixed_rank_file)
                    fixed_ranks = []
                    fixed_totals = []

                    for sort_col in SORT_COLS:
                        rank, total = rank_target_and_total(fixed_rank_rows, pdbid, sort_col)
                ...
                for suffix in SUFFIXES:
                    target_files = get_corr_files(root, map_name, pdbid, suffix)
                    print(f"    {pdbid}_{suffix}: {len(target_files)} files", flush=True)

                    for target_file in target_files:
                        target_rows = read_rows(target_file)

                        original_metrics = compute_original_metrics(target_rows, include_nrmse=True)

                        for m in original_metrics:
                            original_metric_rows_by_map[map_name].append(m)

                        ref_file = get_fixed_reference_file(root, map_name, target_file)

                        if ref_file is None:
                            print("ERROR")
                            print("fixed map_0 reference file missing")
                            print(f"target file : {target_file}")
                            raise SystemExit(1)

                        ref_rows = read_rows(ref_file)
                        fixed_metrics = compute_fixed_map0_metrics(ref_rows, target_rows)
            ...
            n_original = len(original_metric_rows_by_map[map_name])
            n_fixed = len(fixed_metric_rows_by_map[map_name])

            print(f"  finished {map_name}: original_rows={n_original}, fixed_rows={n_fixed}", flush=True)
            print("", flush=True)

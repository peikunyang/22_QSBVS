...
TOTAL_QUBITS = 27
TRANS_CONTROL_SIZE = 512
CONTROL_SIZE = TRANS_CONTROL_SIZE
...
def apply_translation_register(qc, q_p1, q_p2, q_m4, reg):
    add_m4_open(qc, q_m4, reg)
    add_p2(qc, q_p2, reg)
    add_p1(qc, q_p1, reg)


def apply_translation_circuit(qc):
    apply_translation_register(qc, Q_TZ_P1, Q_TZ_P2, Q_TZ_M4, NZ)
    apply_translation_register(qc, Q_TY_P1, Q_TY_P2, Q_TY_M4, NY)
    apply_translation_register(qc, Q_TX_P1, Q_TX_P2, Q_TX_M4, NX)


def shift_from_id(shift_id):
    t_p1 = shift_id & 1
    t_p2 = (shift_id >> 1) & 1
    t_m4 = (shift_id >> 2) & 1

    return (0 if t_m4 == 1 else -4) + 2 * t_p2 + t_p1
...
for tz_id in range(8):
    shift_z = shift_from_id(tz_id)

    for ty_id in range(8):
        shift_y = shift_from_id(ty_id)

        for tx_id in range(8):
            shift_x = shift_from_id(tx_id)

            control_id = tx_id + 8 * ty_id + 64 * tz_id
...
print(f"q18                      = tx +1")
print(f"q19                      = tx +2")
print(f"q20                      = tx -4 open-control")
print(f"q21                      = ty +1")
print(f"q22                      = ty +2")
print(f"q23                      = ty -4 open-control")
print(f"q24                      = tz +1")
print(f"q25                      = tz +2")
print(f"q26                      = tz -4 open-control")

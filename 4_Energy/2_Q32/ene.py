...
OCC_SELECT_SIZE = 16
MAP_SELECT_SIZE = 2
CONTROL_SIZE = BASE_CONTROL_SIZE
...
MAP_DIRS = find_map_dirs(MAP_ROOT_DIR)
...
with torch.no_grad():
    for MAP_DIR in MAP_DIRS:
        ...
        MAP_OUT_DIR = OUT_DIR / MAP_DIR.name
        ...
        for receptor_pdbid in PDBIDS:
            ...
            for map_index, receptor_map_path in enumerate(receptor_map_files):
                receptor_map_name = f"{receptor_pdbid}_{map_index}"

                receptor_map, receptor_vec_norm = load_vector_to_gpu(receptor_map_path)
                receptor_norm_value, receptor_norm_key = find_norm_value(
                    map_norm_table,
                    receptor_map_path,
                )

                RECEPTOR_OUT_DIR = MAP_OUT_DIR / receptor_map_name
                RECEPTOR_OUT_DIR.mkdir(parents=True, exist_ok=True)

                receptor_prob_sum = 0.0
                receptor_inner_min = np.inf
                receptor_inner_max = -np.inf
                receptor_inner_scaled_min = np.inf
                receptor_inner_scaled_max = -np.inf

                for occ_index in range(OCC_SELECT_SIZE):
                    ligand_occ = ligand_occs[occ_index]
                    occ_name = PDBIDS[occ_index]

                    result_file = RECEPTOR_OUT_DIR / f"{receptor_map_name}_{occ_name}.ene"
                    ...
                    scale_factor = receptor_norm_value * ligand_occ_norm_values[occ_index]
                    dots_scaled = dots * scale_factor

                    p0 = (1.0 + dots) / (2.0 * CONTROL_SIZE)
                    p1 = (1.0 - dots) / (2.0 * CONTROL_SIZE)

                    map_col = np.full(BASE_CONTROL_SIZE, receptor_map_name, dtype=object)
                    ...
                    np.savetxt(result_file, data, fmt=fmt, delimiter=" ")

                    file_prob_sum = float(np.sum(p0 + p1))
                    file_inner_min = float(np.min(dots))
                    file_inner_max = float(np.max(dots))
                    file_inner_scaled_min = float(np.min(dots_scaled))
                    file_inner_scaled_max = float(np.max(dots_scaled))

                    receptor_prob_sum += file_prob_sum
                    receptor_inner_min = min(receptor_inner_min, file_inner_min)
                    receptor_inner_max = max(receptor_inner_max, file_inner_max)
                    receptor_inner_scaled_min = min(receptor_inner_scaled_min, file_inner_scaled_min)
                    receptor_inner_scaled_max = max(receptor_inner_scaled_max, file_inner_scaled_max)
                    ...
                    print(f"  receptor map         = {receptor_map_name}")
                    print(f"  ligand occ           = {occ_name}")
                ...
                print(f"finished receptor map  = {receptor_map_name}")
                print(f"  receptor probability = {receptor_prob_sum:.16e}")
                print(f"  receptor min inner   = {receptor_inner_min:.16e}")
                print(f"  receptor max inner   = {receptor_inner_max:.16e}")
                print(f"  receptor min scaled  = {receptor_inner_scaled_min:.16e}")
                print(f"  receptor max scaled  = {receptor_inner_scaled_max:.16e}")

                del receptor_map
                torch.cuda.empty_cache()

        print(f"finished map folder    = {MAP_DIR.name}")
        print(f"  pair count           = {map_pair_count}")
...
print(f"control size per file  = {CONTROL_SIZE}")
...
print(f"total pair count       = {total_pair_count}")

...
def extract_value_from_file(ene_file, pdbid):
...
def extract_value_from_map_dir(map_dir, pdbid):
    target_map = f"{pdbid}_0"
    target_file = map_dir / target_map / f"{target_map}_{pdbid}.ene"

    if target_file.is_file():
        return extract_value_from_file(target_file, pdbid)

    old_style_file = map_dir / f"{pdbid}.ene"

    if old_style_file.is_file():
        return extract_value_from_file(old_style_file, pdbid)

    for ene_file in sorted(map_dir.rglob(args.pattern)):
        value = extract_value_from_file(ene_file, pdbid)
...
for pdbid in PDBIDS:
    ...
    for map_dir in map_dirs:
        value = extract_value_from_map_dir(map_dir, pdbid)
        row_values.append(value)

    results[pdbid] = row_values
...
with open(OUTPUT_FILE, "w", encoding="utf-8") as out:
    for map_i, map_dir in enumerate(map_dirs):
        ...
        for pdbid in PDBIDS:
            values = results[pdbid][map_i]

            if values is not None:
                max_cols = max(max_cols, len(values))

        out.write(f"{map_dir.name}\n")
        out.write(f"{'map':<{MAP_COL_W}s} {'occ':<{OCC_COL_W}s}")

        for j in range(max_cols):
            out.write(f" {f'col{8 + j:02d}':>{VAL_COL_W}s}")
